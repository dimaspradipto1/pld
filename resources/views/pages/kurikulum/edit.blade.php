...
@section('content')
<div class="pagetitle">
    <h1>Edit Matakuliah Kurikulum</h1>
    <nav>
        <ol class="breadcrumb">
...
<div class="card shadow-sm">
    <div class="card-header py-3">
        <h5 class="mb-0 fw-semibold text-dark">
            <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Matakuliah: {{ $kurikulum->nama_mk }}
        </h5>
    </div>
        ...
        <form action="{{ route('kurikulum.update', $kurikulum->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="layanan_id" id="layanan_id" class="form-select @error('layanan_id') is-invalid @enderror" required>
                        ...
                        @foreach($prodis as $prodi)
                            <option value="{{ $prodi->id }}" {{ old('layanan_id', $kurikulum->layanan_id) == $prodi->id ? 'selected' : '' }}>
                                {{ $prodi->judul }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <select name="semester" id="semester" class="form-select @error('semester') is-invalid @enderror" required>
                        @for($s = 1; $s <= 8; $s++)
                            <option value="{{ $s }}" {{ old('semester', $kurikulum->semester) == $s ? 'selected' : '' }}>
                                Semester {{ $s }}
                            </option>
                        @endfor
                    </select>
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="sks" id="sks" class="form-control @error('sks') is-invalid @enderror" value="{{ old('sks', $kurikulum->sks) }}" min="1" max="12" required>
                    @error('sks')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="kode_mk" id="kode_mk" class="form-control @error('kode_mk') is-invalid @enderror" value="{{ old('kode_mk', $kurikulum->kode_mk) }}" placeholder="Contoh: K3-101" required>
                    @error('kode_mk')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="nama_mk" id="nama_mk" class="form-control @error('nama_mk') is-invalid @enderror" value="{{ old('nama_mk', $kurikulum->nama_mk) }}" placeholder="Contoh: Keselamatan & Kesehatan Kerja" required>
                    @error('nama_mk')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select name="kategori" id="kategori" class="form-select @error('kategori') is-invalid @enderror">
                        <option value="Wajib" {{ old('kategori', $kurikulum->kategori) == 'Wajib' ? 'selected' : '' }}>Wajib</option>
                        <option value="Pilihan" {{ old('kategori', $kurikulum->kategori) == 'Pilihan' ? 'selected' : '' }}>Pilihan</option>
                        <option value="Praktikum" {{ old('kategori', $kurikulum->kategori) == 'Praktikum' ? 'selected' : '' }}>Praktikum</option>
                        <option value="Tugas Akhir / Skripsi" {{ old('kategori', $kurikulum->kategori) == 'Tugas Akhir / Skripsi' ? 'selected' : '' }}>Tugas Akhir / Skripsi</option>
                    </select>
                    @error('kategori')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="p-3 rounded-3 bg-light border">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                @if(!empty($kurikulum->file_rps))
                                    <div class="mt-2 p-2 rounded bg-white border d-inline-flex align-items-center gap-2">
                                        <i class="bi bi-file-earmark-pdf text-danger"></i>
                                        <a href="{{ asset('storage/' . $kurikulum->file_rps) }}" target="_blank" class="small text-primary fw-semibold text-decoration-none">
                                            Lihat File RPS Saat Ini
                                        </a>
                                    </div>
                                @endif
                                <small class="text-muted d-block mt-1">Format PDF/DOC (Maks. 10MB). Kosongkan jika tidak diubah.</small>
                                @error('file_rps')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                ...
                                <input type="url" name="link_rps" id="link_rps" class="form-control @error('link_rps') is-invalid @enderror" placeholder="https://drive.google.com/..." value="{{ old('link_rps', $kurikulum->link_rps) }}">
                ...
                <div class="col-12">
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="5" class="form-control tinymce-editor @error('deskripsi') is-invalid @enderror" placeholder="Tuliskan ringkasan materi atau capaian pembelajaran matakuliah ini...">{{ old('deskripsi', $kurikulum->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    <div class="form-check form-switch p-2 bg-light rounded-3 border d-flex align-items-center gap-3">
                        <input class="form-check-input ms-0" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $kurikulum->is_active) ? 'checked' : '' }} style="cursor: pointer;">
            ...
            <div class="mt-4 pt-3 border-top d-flex gap-2">
                <button type="submit" class="btn btn-primary px-4">
                    <i class="bi bi-save me-1"></i> Perbarui Matakuliah
                </button>
